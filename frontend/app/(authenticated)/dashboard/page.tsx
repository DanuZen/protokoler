...
import { Users, CalendarDays, ClipboardList, TrendingUp, Clock, MapPin, ChevronRight, GraduationCap, Handshake, Megaphone, Landmark, ArrowUpRight, CheckCircle2, Timer, TrendingDown, Camera, FileText } from 'lucide-react';
    ...
    <div className="min-h-screen bg-transparent">
      ...
      <div className="relative px-6 md:px-10 pt-24 pb-32 overflow-hidden">
        ...
        <motion.div {...fadeUp(0)} className="relative z-10 flex flex-wrap items-end justify-between gap-6">
          <div>
            <p className="text-[#C9A84C] text-xs font-bold uppercase tracking-[0.25em] mb-3">Sistem Informasi Protokoler</p>
            <h1 className="text-4xl md:text-5xl font-display font-bold text-white tracking-tight leading-tight">Dashboard</h1>
          ...
          <Link href="/kegiatan">
            <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.98 }} className="flex items-center gap-2 bg-[#C9A84C] hover:bg-[#b8963f] text-slate-900 font-bold px-6 py-3 rounded-none transition-all text-sm shadow-lg">
      ...
      <div className="px-6 md:px-10 -mt-16 relative z-20 mb-6">
        <div className="grid gap-4 md:grid-cols-2">
          <Link href="/dokumentasi/dashboard" className="group border border-slate-200 bg-white/95 shadow-sm p-5 flex items-center justify-between gap-4 hover:border-[#C9A84C] transition-colors">
            <div className="flex items-center gap-4 min-w-0">
              <div className="flex h-12 w-12 items-center justify-center bg-slate-950 text-[#C9A84C] shrink-0">
                <Camera className="h-5 w-5" />
              </div>
              <div className="min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Dashboard Baru</p>
                <h3 className="text-lg font-bold text-slate-900">Dokumentasi Kegiatan</h3>
                <p className="text-sm text-slate-500">Upload foto, video, dan dokumen untuk acara yang sudah selesai.</p>
              </div>
            </div>
            <ArrowUpRight className="h-5 w-5 text-slate-300 group-hover:text-[#C9A84C] transition-colors shrink-0" />
          </Link>

          <Link href="/evaluasi/dashboard" className="group border border-slate-200 bg-white/95 shadow-sm p-5 flex items-center justify-between gap-4 hover:border-[#C9A84C] transition-colors">
            <div className="flex items-center gap-4 min-w-0">
              <div className="flex h-12 w-12 items-center justify-center bg-slate-950 text-[#C9A84C] shrink-0">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Dashboard Baru</p>
                <h3 className="text-lg font-bold text-slate-900">Ringkasan Evaluasi</h3>
                <p className="text-sm text-slate-500">Lihat hasil evaluasi protokoler, testimoni tamu, dan feedback admin.</p>
              </div>
            </div>
            <ArrowUpRight className="h-5 w-5 text-slate-300 group-hover:text-[#C9A84C] transition-colors shrink-0" />
          </Link>
      ...
      <div className="bg-slate-50 min-h-screen pt-4">
        ...
        <div className="px-6 md:px-10 -mt-24 relative z-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-0 border border-slate-200 shadow-xl">
            {kpiCards.map((card, i) => (
              <motion.div key={card.label} {...fadeUp(0.1 + i * 0.07)} className={cn('bg-white px-6 py-5 flex items-center gap-5 hover:bg-slate-50 transition-colors', i < kpiCards.length - 1 && 'border-r border-slate-200')}>
                {/* Gold icon box */}
            ...
            <motion.div {...fadeUp(0.35)} className="xl:col-span-2">
              <div className="bg-white border border-slate-200 shadow-sm rounded-none overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100">
                  ...
                  <Link href="/kegiatan" className="flex items-center gap-1.5 text-xs font-bold text-slate-900 hover:text-[#C9A84C] border border-slate-300 hover:border-[#C9A84C] px-3 py-1.5 rounded-none transition-colors">
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      ...
                      {recentActivity.map((k: any, i: number) => {
                        ...
                        return (
                          <motion.tr key={k.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 + i * 0.05 }} className="hover:bg-slate-50/60 transition-colors group">
                            ...
                            <td className="px-4 py-4">
                              <div className="text-sm font-semibold text-slate-800">{new Date(k.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
              ...
              <div className="bg-white border border-slate-200 shadow-sm rounded-none overflow-hidden h-full">
                ...
                <div className="divide-y divide-slate-100">
                  ...
                  {deadlines.map((k: any, i: number) => {
                    ...
                    if (diffDays < 0) {
                      dayLabel = 'Lewat';
                      urgencyBg = 'bg-slate-100 text-slate-400 border border-slate-200';
                      urgencyText = 'text-slate-400';
                    } else if (diffDays === 0) {
                      dayLabel = 'Hari ini';
                      urgencyBg = 'bg-slate-900 text-[#C9A84C] border border-slate-900';
                      urgencyText = 'text-slate-900';
                    } else if (diffDays === 1) {
                      dayLabel = 'Besok';
                      urgencyBg = 'bg-red-50 text-red-600 border border-red-200';
                      urgencyText = 'text-red-600';
                    } else if (diffDays <= 7) {
                      dayLabel = `${diffDays} hari`;
                      urgencyBg = 'bg-amber-50 text-amber-600 border border-amber-200';
                      urgencyText = 'text-amber-600';
                    } else {
                      dayLabel = `${diffDays} hari`;
                      urgencyBg = 'bg-slate-50 text-slate-600 border border-slate-200';
                      urgencyText = 'text-slate-500';
                    }

                    return (
                      <motion.div key={k.id} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.07 }}>
                        <Link href={`/kegiatan/${k.id}`} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors group">
                          ...
                          <div className="flex-shrink-0 text-center w-10 border-r border-slate-100 pr-4">
                            <div className="text-2xl font-extrabold text-slate-900 leading-none font-display">{d.getDate()}</div>
                            <div className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">{d.toLocaleDateString('id-ID', { month: 'short' })}</div>
                          </div>

                          {/* Info */}
                          <div className="flex-1 min-w-0">
                            <div className="font-semibold text-slate-900 text-sm line-clamp-1 group-hover:text-[#C9A84C] transition-colors">{k.nama_kegiatan}</div>
                            <div className="flex items-center gap-1 text-[11px] text-slate-400 mt-0.5">
                              <Clock className="h-3 w-3" />
                              <span>{k.jam_mulai?.slice(0, 5)} WIB</span>
                            </div>
                          ...
                          {/* Urgency badge */}
                          <span className={cn('flex-shrink-0 text-[9px] font-bold px-2 py-1 rounded-none uppercase tracking-wider', urgencyBg)}>{dayLabel}</span>
                        </Link>
                      </motion.div>
                    );
                  })}
