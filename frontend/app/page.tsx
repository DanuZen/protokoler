  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-primary/20 overflow-x-hidden font-sans">
      ...
      <motion.header
        ...
        className={cn('fixed top-0 left-0 right-0 z-50 transition-all duration-300', isScrolled ? 'bg-slate-900/95 backdrop-blur-lg shadow-xl border-b border-white/10 py-3' : 'bg-transparent py-5')}
      >
        <div className="container mx-auto flex items-center justify-between px-6">
          ...
          <nav className="hidden gap-8 lg:flex items-center">
            {['Jadwal', 'Galeri', 'Pengumuman', 'FAQ'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-semibold text-slate-200 transition-colors hover:text-gold">
                {item}
              </a>
            ))}
          </nav>
          <div className="flex items-center gap-4">
            {user ? (
              ...
              <Link href="/auth">
                <Button className="rounded-none shadow-[2px_2px_0px_rgba(0,0,0,0.5)] transition-all px-8 bg-gold text-slate-900 hover:bg-yellow-400 h-11 font-bold uppercase tracking-wider border border-transparent">Masuk Sistem</Button>
              </Link>
            )}
          </div>
        </div>
              ...
              <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="flex flex-col items-center text-center">
                ...
                <motion.h1 variants={fadeUp} className="font-display text-5xl font-black leading-[1.05] tracking-tight md:text-6xl lg:text-[6rem] text-white drop-shadow-xl">
                  ...
                  <span className="relative inline-block mt-4">
                    <span className="relative z-10 text-transparent bg-clip-text bg-gradient-to-r from-gold via-yellow-200 to-amber-500">PROTOKOLER UNP</span>
                    {/* Brutalist underline accent */}
                    <motion.span initial={{ width: 0 }} animate={{ width: '100%' }} transition={{ delay: 0.8, duration: 0.6, ease: 'easeOut' }} className="absolute -bottom-2 left-0 h-3 bg-gold/80 -z-0" />
                  </span>
                </motion.h1>
                ...
                <motion.div variants={fadeUp} className="mt-12 flex items-center justify-center w-full">
                  <Link href={user ? dashboardHref : '/auth'} className="w-full sm:w-auto group">
                    <Button
                      size="lg"
                      className="relative w-full sm:w-auto h-14 rounded-none px-12 text-base shadow-[4px_4px_0px_rgba(0,0,0,0.5)] transition-all bg-gold text-slate-900 hover:bg-yellow-400 hover:translate-y-[2px] hover:shadow-[2px_2px_0px_rgba(0,0,0,0.5)] font-bold uppercase tracking-wider border-2 border-transparent"
                    >
              ...
              {[
                ...
                <motion.div initial={{ opacity: 0, scale: 0.5 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1, type: 'spring' }} key={i} className="text-center px-4">
                  <div className="text-4xl md:text-5xl font-display font-bold mb-3 text-white">
                    {s.value}
                    <span className="text-gold text-2xl md:text-3xl ml-1">{s.suffix}</span>
                  </div>
                ...
                {kegiatanPublik.map((k: any, i: number) => (
                  <motion.div variants={zoomIn} key={k.id || i} className="group rounded-none border border-slate-200 bg-white p-8 transition-all duration-300 hover:border-slate-400 hover:shadow-2xl flex flex-col justify-between">
                    <div>
                      ...
                      <div className="space-y-2 text-sm text-slate-600 mb-6">
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <span>
                            {k.jam_mulai?.slice(0, 5)} - {k.jam_selesai?.slice(0, 5)} WIB
                          </span>
                        </div>
                        <div className="flex items-start gap-2">
                    ...
                    <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                      <span className={cn('text-xs font-bold px-2 py-1 uppercase tracking-widest', k.status === 'berlangsung' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700')}>{k.status}</span>
                    </div>
                  </motion.div>
                ))}
              </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <motion.div
                  ...
                  key={i}
                  className="group relative aspect-[4/5] rounded-none overflow-hidden shadow-xl bg-slate-100 border border-slate-200"
                >
        ...
        <section id="pengumuman" className="bg-slate-50 py-24 md:py-32 relative border-t border-slate-200">
          <div className="container mx-auto px-6">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-2 gap-8">
              {[
                {
                  title: 'Sistem Baru SiProto v1.2 Resmi Diluncurkan',
                  date: '10 Juni 2026',
            ...
            <div className="grid lg:grid-cols-3 gap-16 items-center">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeLeft} className="lg:col-span-1">
                ...
                <h2 className="font-display text-4xl font-bold text-white md:text-5xl mb-6 leading-tight">
                  Suara dari <br />
                  <span className="text-gold">Tim & Tamu</span>
                </h2>
                ...
                <div className="flex gap-2">
                  <Star className="fill-gold text-gold h-7 w-7" />
                  <Star className="fill-gold text-gold h-7 w-7" />
                  <Star className="fill-gold text-gold h-7 w-7" />
                  <Star className="fill-gold text-gold h-7 w-7" />
                  <Star className="fill-gold text-gold h-7 w-7" />
