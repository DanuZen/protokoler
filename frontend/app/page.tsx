...
const faqs = [
  { q: "Bagaimana cara menjadi anggota protokoler di sistem baru ini?", a: "Pendaftaran dapat dilakukan kapan saja melalui portal registrasi. Lengkapi data diri beserta pas foto resmi. Setelah diajukan, akun Anda akan masuk status 'Menunggu Verifikasi' dan akan ditinjau oleh Admin." },
  { q: "Bagaimana sistem perhitungan poin gamifikasi bekerja?", a: "Setiap kegiatan yang berhasil Anda ikuti (hadir dan mengisi evaluasi tepat waktu) akan menambah poin kegiatan Anda. Capai 10 kegiatan untuk medali Silver, dan 20 kegiatan untuk medali Gold." },
  ...
  { q: "Bagaimana cara mendapatkan sertifikat elektronik?", a: "Sertifikat elektronik akan otomatis diterbitkan di profil Anda jika Anda berstatus 'Hadir' pada kegiatan tersebut dan mengisi form evaluasi 3 Tata Protokol dalam batas waktu 1x24 jam." }
];
              ...
              <div className="flex items-center gap-5">
                <div className="hidden md:flex flex-col items-end mr-1">
                  ...
                  <span className="text-[10px] text-gold uppercase tracking-[0.2em] font-bold">
                    {role === "admin" ? "Administrator" : role === "pimpinan" ? "Pimpinan" : "Protokoler"}
                  </span>
                </div>
                <Link href={role === "admin" || role === "pimpinan" ? "/dashboard" : "/beranda"}>
                  <Button className="rounded-none shadow-[2px_2px_0px_rgba(0,0,0,0.5)] transition-all px-8 bg-gold text-slate-900 hover:bg-yellow-400 h-11 font-bold uppercase tracking-wider border border-transparent">
                    Dashboard
                  </Button>
                </Link>
              </div>
      ...
      <main className="relative z-10">
        ...
        <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden min-h-[90vh] flex items-center justify-center">
          ...
          <div className="absolute inset-0 z-0">
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-slate-900/90 via-transparent to-slate-900"></div>
            <div className="absolute inset-0 bg-gradient-to-r from-slate-900/90 via-transparent to-slate-900/90"></div>
          </div>
          
          {/* Glowing Accents */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gold/10 rounded-full blur-[120px] pointer-events-none z-0"></div>
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="flex flex-col items-center justify-center max-w-5xl mx-auto">
              <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="flex flex-col items-center text-center">
                ...
                {/* Main Headline */}
                <motion.h1 variants={fadeUp} className="font-display text-5xl font-black leading-[1.05] tracking-tight md:text-6xl lg:text-[6rem] text-white drop-shadow-xl">
                  STANDAR BARU <br />
                  <span className="relative inline-block mt-4">
                    <span className="relative z-10 text-transparent bg-clip-text bg-gradient-to-r from-gold via-yellow-200 to-amber-500">
                      PROTOKOLER UNP
                    </span>
                    {/* Brutalist underline accent */}
                    <motion.span 
                      initial={{ width: 0 }}
                      animate={{ width: "100%" }}
                      transition={{ delay: 0.8, duration: 0.6, ease: "easeOut" }}
                      className="absolute -bottom-2 left-0 h-3 bg-gold/80 -z-0"
                    />
                  </span>
                </motion.h1>
                ...
                <motion.p variants={fadeUp} className="mt-10 max-w-2xl text-lg md:text-xl text-slate-300 leading-relaxed font-medium drop-shadow-md">
                  Platform manajemen keprotokolan terintegrasi. Dilengkapi sistem <span className="text-white font-bold">absensi geotagging</span>, <span className="text-white font-bold">gamifikasi kinerja</span>, dan penerbitan <span className="text-white font-bold">e-Sertifikat</span> otomatis.
                </motion.p>
                
                {/* Call to Action */}
                <motion.div variants={fadeUp} className="mt-12 flex items-center justify-center w-full">
                  <Link href={user ? (role === "admin" || role === "pimpinan" ? "/dashboard" : "/beranda") : "/auth"} className="w-full sm:w-auto group">
                    <Button size="lg" className="relative w-full sm:w-auto h-14 rounded-none px-12 text-base shadow-[4px_4px_0px_rgba(0,0,0,0.5)] transition-all bg-gold text-slate-900 hover:bg-yellow-400 hover:translate-y-[2px] hover:shadow-[2px_2px_0px_rgba(0,0,0,0.5)] font-bold uppercase tracking-wider border-2 border-transparent">
                      {user ? "Buka Dashboard" : "Bergabung Sekarang"} <ArrowRight className="ml-2 h-5 w-5 transition-transform group-hover:translate-x-1" />
                    </Button>
                  </Link>
                </motion.div>
              </motion.div>
        ...
        {/* Live Stats (Neo-Brutalist adjustments) */}
        <section className="bg-slate-900 py-16 md:py-24 relative overflow-hidden">
          <div className="absolute inset-0 bg-grid-pattern opacity-10"></div>
          <div className="container mx-auto px-6 relative z-10">
            <div className="grid grid-cols-2 gap-10 md:grid-cols-4 divide-x divide-slate-800 text-white py-10">
              {[
                ...
                >
                  <div className="text-4xl md:text-5xl font-display font-bold mb-3 text-white">{s.value}<span className="text-gold text-2xl md:text-3xl ml-1">{s.suffix}</span></div>
        ...
        <section id="jadwal" className="py-24 md:py-32 bg-slate-50 relative">
          <div className="container mx-auto px-6">
            <div className="mb-16 max-w-3xl text-center mx-auto">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-xs font-bold text-gold mb-2 uppercase tracking-[0.2em]">
                Agenda Publik
              </motion.div>
              <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="font-display text-4xl font-bold tracking-tight text-slate-900 md:text-5xl">Kegiatan Mendatang</motion.h2>
            ...
            {isLoading ? (
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {kegiatanPublik.map((k: any, i: number) => (
                  <motion.div variants={zoomIn} key={k.id || i} className="group rounded-none border border-slate-200 bg-white p-8 transition-all duration-300 hover:border-slate-400 hover:shadow-2xl flex flex-col justify-between">
                    <div>
                      <div className="mb-4 inline-flex items-center justify-center rounded-none bg-slate-50 border border-slate-200 text-slate-600 px-3 py-1 text-xs font-bold uppercase tracking-widest">
                        {k.bentuk_kegiatan?.replace(/_/g, " ")}
                      </div>
                      <h3 className="text-xl font-bold text-slate-900 mb-4 leading-snug group-hover:text-gold transition-colors">{k.nama_kegiatan}</h3>
                    ...
                    <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                      <span className={cn(
                        "text-xs font-bold px-2 py-1 uppercase tracking-widest",
                        k.status === "berlangsung" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"
                      )}>
                        {k.status}
                      </span>
                    </div>
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <motion.div 
                  initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: i * 0.2 }}
                  key={i} className="group relative aspect-[4/5] rounded-none overflow-hidden shadow-xl bg-slate-100 border border-slate-200"
                >
                  <Image src={img.src} alt={img.alt} fill className="object-cover transition-transform duration-1000 group-hover:scale-105" />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8 translate-y-4 transition-transform duration-500 group-hover:translate-y-0">
                    ...
                    <div className="w-12 h-1 bg-gold rounded-none opacity-0 transition-opacity duration-500 group-hover:opacity-100"></div>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="container mx-auto px-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeLeft} className="max-w-2xl">
                <div className="text-xs font-bold text-gold mb-2 uppercase tracking-[0.2em]">Informasi</div>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-2 gap-8">
              {[
                { title: "Sistem Baru SiProto v1.2 Resmi Diluncurkan", date: "10 Juni 2026", desc: "Pembaruan besar-besaran dengan penambahan fitur absensi mandiri dan sertifikasi digital otomatis berdasarkan kinerja. Wajib dibaca panduan terbarunya!" },
                ...
                <motion.div variants={fadeUp} key={i} className="flex flex-col sm:flex-row gap-6 sm:gap-8 rounded-none bg-white p-8 border border-slate-200 hover:border-slate-800 transition-all duration-300 hover:-translate-y-1">
                  <div className="flex h-16 w-16 sm:h-20 sm:w-20 shrink-0 items-center justify-center rounded-none bg-slate-50 border border-slate-200 text-slate-800">
                    <Megaphone className="h-8 w-8 sm:h-10 sm:w-10" />
                  </div>
                  <div>
                    <div className="text-sm font-bold text-slate-500 mb-2 uppercase tracking-widest">{news.date}</div>
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="grid lg:grid-cols-3 gap-16 items-center">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeLeft} className="lg:col-span-1">
                <div className="text-xs font-bold text-gold mb-2 uppercase tracking-[0.2em]">Testimoni</div>
                <h2 className="font-display text-4xl font-bold text-white md:text-5xl mb-6 leading-tight">Suara dari <br/><span className="text-gold">Tim & Tamu</span></h2>
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="lg:col-span-2 grid md:grid-cols-2 gap-8">
                {[
                  ...
                  <motion.div variants={fadeUp} key={i} className="p-10 rounded-none bg-white/5 backdrop-blur-md border border-white/10 relative hover:bg-white/10 transition-colors duration-300">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gold font-bold mt-1 uppercase tracking-wider">{testi.role}</p>
                    </div>
                  </motion.div>
                ))}
              </motion.div>
        ...
        <section id="faq" className="py-24 md:py-32 bg-white relative">
          <div className="container mx-auto px-6 max-w-4xl">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center mb-16">
              <div className="text-xs font-bold text-gold mb-2 uppercase tracking-[0.2em]">FAQ</div>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="space-y-4">
              {faqs.map((faq, i) => {
                ...
                return (
                  <motion.div variants={fadeUp} key={i} className={cn("bg-white rounded-none overflow-hidden transition-all duration-300 border", isOpen ? "border-slate-800 shadow-md" : "border-slate-200 hover:border-slate-400")}>
                    <button onClick={() => setOpenFaq(isOpen ? null : i)} className="w-full flex items-center justify-between p-6 text-left focus:outline-none">
                      ...
                      <div className={cn("shrink-0 h-10 w-10 flex items-center justify-center transition-all duration-500 rounded-none", isOpen ? "bg-slate-900 text-white rotate-180" : "bg-slate-100 text-slate-500")}>
                    ...
                    <AnimatePresence>
                      {isOpen && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="overflow-hidden">
                          <div className="p-6 pt-0 text-slate-600 text-base md:text-lg leading-relaxed border-t border-slate-100 mt-2">
                            {faq.a}
                          </div>
                        </motion.div>
                      )}
      ...
      <footer className="border-t border-white/10 bg-slate-900 py-12 relative z-20">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            ...
            <span className="font-display text-xl font-bold text-white tracking-widest">SIPROTO</span>
          </div>
          <p className="text-sm text-slate-500 font-medium">
            © {new Date().getFullYear()} Unit Protokoler Universitas. Hak Cipta Dilindungi.
          </p>
        </div>
      </footer>
    </div>
