...
export const Route = createFileRoute("/_authenticated/kegiatan/$id")({ component: Page });
...
function Page() {
  ...
  return (
    <div className="space-y-6">
      <Link to="/kegiatan" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground">
        ...
        <div className="grid gap-4">
          <div>
            ...
            <Select value={mahasiswaId} onValueChange={setMahasiswaId}>
              ...
              <SelectContent>{mhs?.map((m) => <SelectItem key={m.id} value={m.id}>{m.nama_lengkap} ({m.nim})</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div>
            <Label>Peran</Label>
